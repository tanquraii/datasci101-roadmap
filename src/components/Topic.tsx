  ...
  return (
    <div className="border border-[#251E38] rounded-lg overflow-hidden bg-[#181622] shadow-sm mb-4 transition-all">
      <button
        ...
        className="w-full text-left p-4 bg-[#181622] hover:bg-[#251E38]/50 flex items-center justify-between transition-colors focus:outline-none"
      >
        <div>
      ...
      {isOpen && (
        <div className="p-4 border-t border-[#251E38] bg-[#181622]">
          ...
          <h4 className="text-xs font-bold text-[#8C8A99] uppercase tracking-wider mb-2">
            Resources
          </h4>
          <ul className="space-y-2">
            {urls.map((url, index) => {
              ...
              return (
                <li
                  key={index}
                  className="flex items-center text-sm text-white bg-[#251E38]/40 p-2 rounded border border-[#251E38] gap-2"
                >
                  <span className="w-2 h-2 rounded-full bg-[#8E52FF] shrink-0"></span>

                  {currentAuthor && (
                    <span className="text-[#8E52FF] font-semibold shrink-0">
                      {currentAuthor}:
                    </span>
                  )}
                  ...
                  <a
                    ...
                    className="hover:text-[#8E52FF] hover:underline truncate"
                  >
                    {url}
                  </a>
                </li>
